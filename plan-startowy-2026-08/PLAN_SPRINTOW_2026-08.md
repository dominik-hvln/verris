...
Domknięcie **wszystkich** luk z macierzy to **2762 h** — przy 30 h tygodniowo około **21 miesięcy pracy solo, bez jednego przychodu po drodze**. Taki plan nie jest planem startu, tylko sposobem, żeby nigdy nie wystartować.

Dlatego praca dzieli się na dwie części: **23 sprintów do startu** (1178 h) oraz roadmapę po starcie (1584 h, 81 pozycji) rozpisaną na epiki kwartalne.
...
*Sprinty 4–8 · 314 h · 2026-09-21 – 2026-10-23*
...
| `PB-38` | Najnowsze stabilne wersje narzędzi i bibliotek przed startem | 30 | WYSOKI | Decyzja właściciela 2026-09-26 (formularz): Node 24 teraz, 26 po wejściu w LTS; TypeScript 6.0 teraz, 7.x gdy typescript-eslint go obsłuży; MinIO zost |
| `PB-39` | API i biblioteki jako ESM (Vitest, Prisma prisma-client) | 14 | WYSOKI | Decyzja właściciela 2026-09-26 (formularz): przechodzimy teraz, przed falą 3. ESM to domyślny format NestJS 12; znika eksperymentalna flaga Jesta; pri |
...
- `PB-38` — Bramka (lint, typecheck, testy) i testy integracyjne zielone na nowych wersjach; obrazy budują się w CI; wdrożenie produkcyjne bez błędów; Dependabot i strażnik EOL działają.
- `PB-39` — Bramka i testy integracyjne zielone na Vitest; obraz API buduje się w CI; wdrożenie produkcyjne bez błędów; główne ścieżki działają po wdrożeniu.
- **Cały sprint** — `audyt/dane/macierz.csv` zaktualizowana (uzasadnienie w „Uwagach”), widoki przebudowane, decyzje dopisane do `docs/VERRIS.md`.
...
81 pozycji, 1584 h. Epiki, nie sprinty — kolejność zweryfikujemy danymi od pierwszych klientów.
...
| `E-09` | Pokrycie testowe warstw krytycznych | WYSOKI | ciągłe | 1 | 40 | Realizowane równolegle z każdą fazą, nie jako osobny projekt. Zasada: każda naprawiona pozycja dostaje test, który najpierw czerwieni się na starym kodzie. |
...
- **E-09 Pokrycie testowe warstw krytycznych** (40 h) — Testy integracyjne API, moduł auth, klient KSeF, ścieżka backup/restore, DirectAdminService, panele frontowe.
