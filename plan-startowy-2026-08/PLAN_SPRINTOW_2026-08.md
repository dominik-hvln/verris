...
Domknięcie **wszystkich** luk z macierzy to **2785 h** — przy 30 h tygodniowo około **22 miesięcy pracy solo, bez jednego przychodu po drodze**. Taki plan nie jest planem startu, tylko sposobem, żeby nigdy nie wystartować.

Dlatego praca dzieli się na dwie części: **22 sprintów do startu** (979 h) oraz roadmapę po starcie (1806 h, 91 pozycji) rozpisaną na epiki kwartalne.
...
91 pozycji, 1806 h. Epiki, nie sprinty — kolejność zweryfikujemy danymi od pierwszych klientów.

| ID | Epik | Priorytet | Kwartał | Pozycji | h | Dlaczego teraz, a nie wcześniej |
|---|---|---|---|---|---|---|
| `E-01` | Runtime, pliki i diagnostyka | WYSOKI | Q1 2027 | 21 | 358 | Najczęstsze źródło zgłoszeń w pierwszych miesiącach każdego hostingu. Logi WWW ma pięć z pięciu badanych hostingów PL — bez nich klient nie zdiagnozuje własnej strony i pisze do nas. |
...
| `E-03` | WordPress Toolkit | WYSOKI | Q2 2027 | 6 | 134 | Cztery z pięciu hostingów PL mają automatyczne aktualizacje WordPressa. Staging już mamy i jest przewagą — reszta toolkitu ją domyka. |
...
| `E-13` | Automatyzacja: API zapisu i webhooki | ŚREDNI | Q3 2027 | 6 | 80 | Żaden hosting PL nie ma publicznego API — mamy przewagę, która dziś obejmuje pięć GET-ów przy opisie obiecującym CI/CD i Terraform. |
...
- **E-01 Runtime, pliki i diagnostyka** (358 h) — php.ini i rozszerzenia PHP z panelu, logi dostępu i błędów WWW, import/eksport bazy, spakowanie archiwum, SSH i klucze SSH dla hostingu, podgląd zajętości katalogów.
...
- **E-03 WordPress Toolkit** (134 h) — Automatyczne aktualizacje, aktualizacje wtyczek i motywów, klonowanie między domenami, hardening, skan podatności, tryb konserwacji, masowe zarządzanie.
...
- **E-13 Automatyzacja: API zapisu i webhooki** (80 h) — Rozszerzenie publicznego API o operacje zapisu, webhooki dla klienta, edycja crona, cron z wyborem wersji PHP, podgląd wyniku wykonania.
