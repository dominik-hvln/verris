...
export class HostingService {
  static async getDomains(): Promise<Domain[]> {
    await delay(800);
    return [
      { id: '1', name: 'ekohost.pl', status: 'ACTIVE', expiryDate: '2027-04-01', autoRenew: true },
  ...
  static async getDnsRecords(domain: string): Promise<DnsRecord[]> {
    ...
    return [
      ...
      { id: '2', type: 'CNAME', name: 'www', value: 'ekohost.pl', ttl: 3600 },
      { id: '3', type: 'MX', name: '@', value: 'mail.ekohost.pl (10)', ttl: 3600 },
  ...
  static async getDatabases(): Promise<Database[]> {
    await delay(700);
    return [
      { id: '1', name: 'ekohost_wp_main', user: 'ekohost_dbu', sizeMb: 124.5 },
      { id: '2', name: 'ekohost_shop', user: 'ekohost_shop_u', sizeMb: 45.2 },
    ];
  }
  ...
  static async getSslCertificates(): Promise<SslCertificate[]> {
    await delay(500);
    return [
      { id: '1', domain: 'ekohost.pl', issuer: "Let's Encrypt", expiryDate: '2026-07-01', status: 'VALID', isLetsEncrypt: true },
      { id: '2', domain: 'panel.ekohost.pl', issuer: "Let's Encrypt", expiryDate: '2026-06-15', status: 'VALID', isLetsEncrypt: true },
    ];
  }
}
