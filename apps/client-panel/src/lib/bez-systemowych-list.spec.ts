...
/** Systemowe pole wyboru wygląda inaczej w każdej przeglądarce — tylko `Checkbox` z `@/components/panel/checkbox`. */
it('żaden plik panelu nie renderuje systemowego pola wyboru (checkbox)', () => {
  ...
  const przejdz = (dir: string) => {
    for (const n of readdirSync(dir)) {
      const p = join(dir, n);
      if (statSync(p).isDirectory()) przejdz(p);
      else if (n.endsWith('.tsx') && !p.endsWith(join('components', 'panel', 'checkbox.tsx')) && /type=["']checkbox["']/.test(readFileSync(p, 'utf8'))) {
        trafienia.push(p.split('/src/')[1]!);
      }
    }
  };
