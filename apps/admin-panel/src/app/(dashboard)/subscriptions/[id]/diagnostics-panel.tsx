...
/** ADM-2 — panel „Diagnozuj" w szczegółach subskrypcji (admin/staff). */
export function DiagnosticsPanel({ subscriptionId }: { subscriptionId: string }) {
  ...
  const run = () =>
    start(async () => {
      setError(null);
      const res = await runDiagnosticsAction(subscriptionId);
      ...
      <div className="flex items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted-foreground">
            Składa subskrypcję, konto DA, węzeł i checki DNS/SSL/poczty/backupu w jeden obraz z sugerowaną akcją.
          </p>
        </div>
        <button
          type="button"
          onClick={run}
          disabled={pending}
          className="shrink-0 rounded-lg bg-indigo-500 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-400 disabled:opacity-50"
        >
