    ...
    <div className="space-y-6 p-6">
      <header>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          Akceptacja lub odrzucenie zgłoszeń do programu poleceń. Po akceptacji klient otrzymuje kod
          polecający i może zbierać punkty EKO za polecenia.
        </p>
      </header>
              ...
              rows.map((row) => (
                <tr key={row.id} className="border-b border-white/5 last:border-0">
                  ...
                  <td className="px-4 py-3 text-muted-foreground whitespace-nowrap">
                    ...
                    {row.termsVersion ? (
                      <p className="text-[10px] text-neutral-500">Regulamin v{row.termsVersion}</p>
                    ) : null}
                  </td>
                  <td className="px-4 py-3">
                    <span
                    ...
                    {row.reviewedAt ? (
                      <p className="mt-1 text-[10px] text-neutral-500">
                        {new Date(row.reviewedAt).toLocaleString("pl-PL")}
                      </p>
                    ) : null}
                    {row.reviewNote ? (
