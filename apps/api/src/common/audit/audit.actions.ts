/**
 * Centralized list of audit log action codes.
 *
 * `AuditLog.action` is a free-form `String` in the DB schema, but in code we
 * keep all known action codes here so admin panel filters and analytics can
 * group them by category. Adding a new action = adding a constant here +
 * extending the `RODO_ACTIONS` / `SECURITY_ACTIONS` etc. set if relevant.
 */

// ---------------------------------------------------------------------------
// RODO / GDPR (Sprint 1)
// ---------------------------------------------------------------------------

export const RodoActions = {
  CONSENT_GRANTED: 'CONSENT_GRANTED',
  CONSENT_WITHDRAWN: 'CONSENT_WITHDRAWN',
  RE_CONSENT_GRANTED: 'RE_CONSENT_GRANTED',

  MARKETING_OPT_IN: 'MARKETING_OPT_IN',
  MARKETING_OPT_OUT: 'MARKETING_OPT_OUT',

  DATA_EXPORT_REQUESTED: 'DATA_EXPORT_REQUESTED',
  DATA_EXPORT_GENERATED: 'DATA_EXPORT_GENERATED',
  DATA_EXPORT_DOWNLOADED: 'DATA_EXPORT_DOWNLOADED',
  DATA_EXPORT_EXPIRED: 'DATA_EXPORT_EXPIRED',
  DATA_EXPORT_FAILED: 'DATA_EXPORT_FAILED',

  ACCOUNT_DELETION_REQUESTED: 'ACCOUNT_DELETION_REQUESTED',
  ACCOUNT_DELETION_CANCELLED: 'ACCOUNT_DELETION_CANCELLED',
  ACCOUNT_ANONYMIZED: 'ACCOUNT_ANONYMIZED',
  /** DirectAdmin-side hard purge of a hosting account (30 days post-anon). */
  ACCOUNT_DA_PURGED: 'ACCOUNT_DA_PURGED',

  LEGAL_DOC_VERSION_PUBLISHED: 'LEGAL_DOC_VERSION_PUBLISHED',
  LEGAL_DOC_VERSION_RETIRED: 'LEGAL_DOC_VERSION_RETIRED',

  /** B2B clients accepting the current DPA version. */
  DPA_ACCEPTED: 'DPA_ACCEPTED',
  /** Successful issuance of a downloadable signed DPA PDF. */
  DPA_PDF_GENERATED: 'DPA_PDF_GENERATED',

  ADMIN_FORCED_DATA_EXPORT: 'ADMIN_FORCED_DATA_EXPORT',
  ADMIN_FORCED_ACCOUNT_ANONYMIZED: 'ADMIN_FORCED_ACCOUNT_ANONYMIZED',

  RETENTION_PURGE: 'RETENTION_PURGE',
} as const;

export type RodoAction = (typeof RodoActions)[keyof typeof RodoActions];

/**
 * Set of all RODO action codes. Used by admin panel filter
 * `?category=RODO` to scope the audit log view.
 */
export const RODO_ACTION_SET: ReadonlySet<string> = new Set(Object.values(RodoActions));

export function isRodoAction(action: string): boolean {
  return RODO_ACTION_SET.has(action);
}

// ---------------------------------------------------------------------------
// Support / BOK (Sprint 3)
// ---------------------------------------------------------------------------

export const SupportActions = {
  /** Staff/admin uruchomił diagnostykę DNS+TLS z profilu klienta. */
  STAFF_DNS_TLS_DIAGNOSTIC: 'STAFF_DNS_TLS_DIAGNOSTIC',
} as const;

// ---------------------------------------------------------------------------
// Admin operacyjny (Sprint 4)
// ---------------------------------------------------------------------------

export const AdminCustomerActions = {
  CUSTOMER_LOGIN_BLOCK_UPDATED: 'ADMIN_CUSTOMER_LOGIN_BLOCK_UPDATED',
  CUSTOMER_INTERNAL_NOTE_UPDATED: 'ADMIN_CUSTOMER_INTERNAL_NOTE_UPDATED',
  // PROD-03 — oznaczenie konta jako wewnętrzne (testowe).
  CUSTOMER_INTERNAL_FLAG_UPDATED: 'ADMIN_CUSTOMER_INTERNAL_FLAG_UPDATED',
  // L-11 — dostawca/model poziomów asystenta AI, ceny i limit klienta.
  AI_SETTINGS_UPDATED: 'ADMIN_AI_SETTINGS_UPDATED',
  CUSTOMER_EMAIL_CHANGED: 'ADMIN_CUSTOMER_EMAIL_CHANGED',
  CUSTOMER_PASSWORD_RESET_BY_ADMIN: 'ADMIN_CUSTOMER_PASSWORD_RESET_BY_ADMIN',
  CUSTOMER_GRAFANA_ACCESS_TOGGLED: 'ADMIN_CUSTOMER_GRAFANA_ACCESS_TOGGLED',
  CUSTOMER_CREATED_BY_OPERATOR: 'ADMIN_CUSTOMER_CREATED_BY_OPERATOR',
} as const;

/**
 * Akcje powiązane z planami produktowymi i Stripe Price IDs (Sprint 4 / R-05).
 */
export const AdminPlanActions = {
  PLAN_CREATED: 'PLAN_CREATED',
  PLAN_UPDATED: 'PLAN_UPDATED',
  PLAN_DEACTIVATED: 'PLAN_DEACTIVATED',
} as const;

/**
 * Akcje powiązane z fakturowaniem widziane od strony admina (Sprint 4 / R-10).
 */
export const AdminInvoiceActions = {
  INVOICE_PDF_DOWNLOADED_BY_ADMIN: 'ADMIN_INVOICE_PDF_DOWNLOADED',
  INVOICES_CSV_EXPORTED: 'ADMIN_INVOICES_CSV_EXPORTED',
} as const;

/**
 * Akcje powiązane z infrastrukturą węzłów (maintenance mode, A-08).
 */
export const AdminNodeActions = {
  NODE_MAINTENANCE_MODE_TOGGLED: 'ADMIN_NODE_MAINTENANCE_MODE_TOGGLED',
  MAINTENANCE_WINDOW_CREATED: 'ADMIN_MAINTENANCE_WINDOW_CREATED',
  MAINTENANCE_WINDOW_UPDATED: 'ADMIN_MAINTENANCE_WINDOW_UPDATED',
  STATUS_WEBHOOK_ENDPOINT_CREATED: 'ADMIN_STATUS_WEBHOOK_ENDPOINT_CREATED',
  STATUS_WEBHOOK_ENDPOINT_UPDATED: 'ADMIN_STATUS_WEBHOOK_ENDPOINT_UPDATED',
} as const;

export const ProvisioningActions = {
  PROVISIONING_JOB_QUEUED: 'PROVISIONING_JOB_QUEUED',
  PROVISIONING_JOB_STARTED: 'PROVISIONING_JOB_STARTED',
  PROVISIONING_JOB_RETRYING: 'PROVISIONING_JOB_RETRYING',
  PROVISIONING_JOB_FAILED: 'PROVISIONING_JOB_FAILED',
  PROVISIONING_JOB_COMPLETED: 'PROVISIONING_JOB_COMPLETED',
  PROVISIONING_JOB_RETRIED_BY_ADMIN: 'PROVISIONING_JOB_RETRIED_BY_ADMIN',
  /**
   * X-32 — martwy job usunięty z kolejki ręką operatora.
   *
   * Osobna akcja, a nie wariant retry: w raportach odrzucenie i ponowienie
   * to dwie różne decyzje i nie wolno im się zlewać. Wpis niesie subskrypcję,
   * joba, liczbę prób, ostatni błąd i POWÓD podany przez człowieka.
   */
  PROVISIONING_JOB_DISCARDED_BY_ADMIN: 'PROVISIONING_JOB_DISCARDED_BY_ADMIN',
} as const;

/**
 * Per-account hosting resource changes performed from the client panel
 * (databases, FTP, mailboxes, cron, files). Destructive ones in particular
 * give an audit trail for security/RODO.
 */
export const HostingResourceActions = {
  HOSTING_DB_CREATED: 'HOSTING_DB_CREATED',
  HOSTING_DB_DELETED: 'HOSTING_DB_DELETED',
  HOSTING_FTP_CREATED: 'HOSTING_FTP_CREATED',
  HOSTING_FTP_DELETED: 'HOSTING_FTP_DELETED',
  HOSTING_FTP_PASSWORD_CHANGED: 'HOSTING_FTP_PASSWORD_CHANGED',
  HOSTING_EMAIL_CREATED: 'HOSTING_EMAIL_CREATED',
  HOSTING_EMAIL_DELETED: 'HOSTING_EMAIL_DELETED',
  HOSTING_EMAIL_PASSWORD_CHANGED: 'HOSTING_EMAIL_PASSWORD_CHANGED',
  HOSTING_EMAIL_QUOTA_CHANGED: 'HOSTING_EMAIL_QUOTA_CHANGED',
  HOSTING_CRON_CREATED: 'HOSTING_CRON_CREATED',
  HOSTING_CRON_DELETED: 'HOSTING_CRON_DELETED',
  HOSTING_FILE_DELETED: 'HOSTING_FILE_DELETED',
  HOSTING_FILE_WRITTEN: 'HOSTING_FILE_WRITTEN',
  HOSTING_FILE_RENAMED: 'HOSTING_FILE_RENAMED',
  HOSTING_FILE_UPLOADED: 'HOSTING_FILE_UPLOADED',
  HOSTING_SUBDOMAIN_CREATED: 'HOSTING_SUBDOMAIN_CREATED',
  HOSTING_SUBDOMAIN_DELETED: 'HOSTING_SUBDOMAIN_DELETED',
  HOSTING_EMAIL_FORWARD_CREATED: 'HOSTING_EMAIL_FORWARD_CREATED',
  HOSTING_EMAIL_FORWARD_DELETED: 'HOSTING_EMAIL_FORWARD_DELETED',
  HOSTING_AUTORESPONDER_SET: 'HOSTING_AUTORESPONDER_SET',
  HOSTING_AUTORESPONDER_DELETED: 'HOSTING_AUTORESPONDER_DELETED',
  HOSTING_REDIRECT_CREATED: 'HOSTING_REDIRECT_CREATED',
  HOSTING_REDIRECT_DELETED: 'HOSTING_REDIRECT_DELETED',
  HOSTING_DIRPROTECT_SET: 'HOSTING_DIRPROTECT_SET',
  HOSTING_DIRPROTECT_DELETED: 'HOSTING_DIRPROTECT_DELETED',
  HOSTING_ADDON_DOMAIN_CREATED: 'HOSTING_ADDON_DOMAIN_CREATED',
  HOSTING_ADDON_DOMAIN_DELETED: 'HOSTING_ADDON_DOMAIN_DELETED',
  HOSTING_DOMAIN_POINTER_CREATED: 'HOSTING_DOMAIN_POINTER_CREATED',
  HOSTING_DOMAIN_POINTER_DELETED: 'HOSTING_DOMAIN_POINTER_DELETED',
  HOSTING_HTACCESS_RULES_SET: 'HOSTING_HTACCESS_RULES_SET',
  HOSTING_QUOTA_ALERT: 'HOSTING_QUOTA_ALERT',
  HOSTING_CATCHALL_SET: 'HOSTING_CATCHALL_SET',
  HOSTING_DKIM_ENABLED: 'HOSTING_DKIM_ENABLED',
  HOSTING_FILE_DOWNLOADED: 'HOSTING_FILE_DOWNLOADED',
  HOSTING_SPAMFILTER_SET: 'HOSTING_SPAMFILTER_SET',
  HOSTING_DB_ACCESSHOST_ADDED: 'HOSTING_DB_ACCESSHOST_ADDED',
  HOSTING_DB_ACCESSHOST_DELETED: 'HOSTING_DB_ACCESSHOST_DELETED',
  // S-1 — samoobsługowe odtwarzanie z kopii off-site (rclone) w panelu klienta.
  HOSTING_OFFSITE_LIST_QUEUED: 'HOSTING_OFFSITE_LIST_QUEUED',
  HOSTING_OFFSITE_FETCH_QUEUED: 'HOSTING_OFFSITE_FETCH_QUEUED',
  // H-16 — odtworzenie konta z kopii off-site na innym węźle (operator) i przepięcie konta.
  ACCOUNT_RESTORE_TO_NODE_QUEUED: 'ACCOUNT_RESTORE_TO_NODE_QUEUED',
  ACCOUNT_MOVED_TO_NODE: 'ACCOUNT_MOVED_TO_NODE',
  // Menedżer plików — operacje dodane w lipcu 2026 (kopiuj / przenieś / rozpakuj / chmod).
  HOSTING_FILE_COPIED: 'HOSTING_FILE_COPIED',
  HOSTING_FILE_MOVED: 'HOSTING_FILE_MOVED',
  HOSTING_FILE_EXTRACTED: 'HOSTING_FILE_EXTRACTED',
  HOSTING_FILE_COMPRESSED: 'HOSTING_FILE_COMPRESSED',
  HOSTING_FILE_CHMOD_SET: 'HOSTING_FILE_CHMOD_SET',
  // Dodatkowi użytkownicy baz MySQL (SPRINT-1a).
  HOSTING_DB_USER_CREATED: 'HOSTING_DB_USER_CREATED',
  HOSTING_DB_USER_DELETED: 'HOSTING_DB_USER_DELETED',
  HOSTING_DB_USER_PASSWORD_CHANGED: 'HOSTING_DB_USER_PASSWORD_CHANGED',
  // Jednorazowy link logowania klienta do phpMyAdmin / webmaila (SPRINT-1c).
  HOSTING_SSO_URL_CREATED: 'HOSTING_SSO_URL_CREATED',
  // Wersja PHP ustawiana per domena, obok selektora per konto (FALA-2b).
  HOSTING_DOMAIN_PHP_SET: 'HOSTING_DOMAIN_PHP_SET',
  HOSTING_PHP_INI_SET: 'HOSTING_PHP_INI_SET',
  // A-06: katalog główny domeny (DocumentRoot przez Custom HTTPD DA).
  HOSTING_DOCROOT_SET: 'HOSTING_DOCROOT_SET',
  // F-06: DNSSEC strefy (CMD_API_DNS_ADMIN action=dnssec).
  HOSTING_DNSSEC_ENABLED: 'HOSTING_DNSSEC_ENABLED',
  HOSTING_DNSSEC_DISABLED: 'HOSTING_DNSSEC_DISABLED',
  HOSTING_DB_EXPORT_QUEUED: 'HOSTING_DB_EXPORT_QUEUED',
  HOSTING_DB_IMPORT_QUEUED: 'HOSTING_DB_IMPORT_QUEUED',
  HOSTING_FILE_RESTORE_QUEUED: 'HOSTING_FILE_RESTORE_QUEUED',
  HOSTING_SSH_ENABLE_QUEUED: 'HOSTING_SSH_ENABLE_QUEUED',
  HOSTING_SSH_DISABLE_QUEUED: 'HOSTING_SSH_DISABLE_QUEUED',
  HOSTING_SSH_KEYS_QUEUED: 'HOSTING_SSH_KEYS_QUEUED',
  HOSTING_WP_UPDATE_QUEUED: 'HOSTING_WP_UPDATE_QUEUED',
  HOSTING_WP_AUTO_UPDATE_SET: 'HOSTING_WP_AUTO_UPDATE_SET',
  HOSTING_WP_CACHE_QUEUED: 'HOSTING_WP_CACHE_QUEUED',
  HOSTING_WP_HARDEN_QUEUED: 'HOSTING_WP_HARDEN_QUEUED',
  HOSTING_MALWARE_SCAN_QUEUED: 'HOSTING_MALWARE_SCAN_QUEUED',
  HOSTING_REDIS_ENABLE_QUEUED: 'HOSTING_REDIS_ENABLE_QUEUED',
  HOSTING_REDIS_DISABLE_QUEUED: 'HOSTING_REDIS_DISABLE_QUEUED',
  HOSTING_MEMCACHED_ENABLE_QUEUED: 'HOSTING_MEMCACHED_ENABLE_QUEUED',
  HOSTING_MEMCACHED_DISABLE_QUEUED: 'HOSTING_MEMCACHED_DISABLE_QUEUED',
  HOSTING_PGSQL_CREATE_QUEUED: 'HOSTING_PGSQL_CREATE_QUEUED',
  HOSTING_PGSQL_DELETE_QUEUED: 'HOSTING_PGSQL_DELETE_QUEUED',
  HOSTING_PGSQL_PASSWORD_QUEUED: 'HOSTING_PGSQL_PASSWORD_QUEUED',
  HOSTING_IMAGES_OPTIMIZE_QUEUED: 'HOSTING_IMAGES_OPTIMIZE_QUEUED',
  HOSTING_GIT_CLONE_QUEUED: 'HOSTING_GIT_CLONE_QUEUED',
  HOSTING_GIT_PULL_QUEUED: 'HOSTING_GIT_PULL_QUEUED',
  HOSTING_GIT_WEBHOOK_CREATED: 'HOSTING_GIT_WEBHOOK_CREATED',
  HOSTING_SITE_CLONE_QUEUED: 'HOSTING_SITE_CLONE_QUEUED',
  HOSTING_HTACCESS_QUEUED: 'HOSTING_HTACCESS_QUEUED',
  // B-08/B-09 — operacja na aplikacji Node.js/Python (CloudLinux Selector).
  HOSTING_APP_SELECTOR_QUEUED: 'HOSTING_APP_SELECTOR_QUEUED',
  HOSTING_DB_MAINTENANCE_QUEUED: 'HOSTING_DB_MAINTENANCE_QUEUED',
  HOSTING_DB_PRIVILEGES_QUEUED: 'HOSTING_DB_PRIVILEGES_QUEUED',
} as const;

export const MigrationActions = {
  MIGRATION_BUNDLE_QUEUED: 'MIGRATION_BUNDLE_QUEUED',
  MIGRATION_BUNDLE_PICKED_UP: 'MIGRATION_BUNDLE_PICKED_UP',
  MIGRATION_SECRETS_REVEALED: 'MIGRATION_SECRETS_REVEALED',
  MIGRATION_WORKER_JOB_QUEUED: 'MIGRATION_WORKER_JOB_QUEUED',
  MIGRATION_WORKER_JOB_STARTED: 'MIGRATION_WORKER_JOB_STARTED',
  MIGRATION_WORKER_JOB_COMPLETED: 'MIGRATION_WORKER_JOB_COMPLETED',
  MIGRATION_WORKER_JOB_FAILED: 'MIGRATION_WORKER_JOB_FAILED',
  MIGRATION_POST_CHECK_COMPLETED: 'MIGRATION_POST_CHECK_COMPLETED',
  // Migrator v2 (self-service A→Z)
  MIGRATION_DISCOVERY_RUN: 'MIGRATION_DISCOVERY_RUN',
  MIGRATION_PREFLIGHT_RUN: 'MIGRATION_PREFLIGHT_RUN',
  MIGRATION_ESCALATED: 'MIGRATION_ESCALATED',
  MIGRATION_ATTENTION_RESOLVED: 'MIGRATION_ATTENTION_RESOLVED',
  MIGRATION_WORKER_JOB_RETRIED: 'MIGRATION_WORKER_JOB_RETRIED',
  MIGRATION_DELTA_SYNC_QUEUED: 'MIGRATION_DELTA_SYNC_QUEUED',
  MIGRATION_CUTOVER_REQUESTED: 'MIGRATION_CUTOVER_REQUESTED',
  MIGRATION_CUTOVER_DNS_APPLIED: 'MIGRATION_CUTOVER_DNS_APPLIED',
  MIGRATION_SECRETS_PURGED: 'MIGRATION_SECRETS_PURGED',
  MIGRATION_CANCELED: 'MIGRATION_CANCELED',
} as const;

export const TicketOpsActions = {
  TICKET_SLA_UPDATED: 'TICKET_SLA_UPDATED',
  TICKET_ESCALATED: 'TICKET_ESCALATED',
  TICKET_RUNBOOK_APPLIED: 'TICKET_RUNBOOK_APPLIED',
  CUSTOMER_RISK_FLAG_UPDATED: 'CUSTOMER_RISK_FLAG_UPDATED',
} as const;

export const ControlPlaneMailActions = {
  MAILBOX_CREATED: 'MAILBOX_CREATED',
  MAILBOX_UPDATED: 'MAILBOX_UPDATED',
  MAILBOX_PASSWORD_RESET: 'MAILBOX_PASSWORD_RESET',
  MAIL_ALIAS_ADDED: 'MAIL_ALIAS_ADDED',
  MAIL_ALIAS_REMOVED: 'MAIL_ALIAS_REMOVED',
  MAIL_POSTFIX_MAPS_SYNCED: 'MAIL_POSTFIX_MAPS_SYNCED',
  SYSTEM_ADDRESS_CHANGED: 'SYSTEM_ADDRESS_CHANGED',
  MAIL_FORWARD_ADDED: 'MAIL_FORWARD_ADDED',
  MAIL_FORWARD_REMOVED: 'MAIL_FORWARD_REMOVED',
  MAIL_FORWARD_CONFIRMED: 'MAIL_FORWARD_CONFIRMED',
  MAILBOX_IMPORT: 'MAILBOX_IMPORT',
} as const;

export const ProductOpsActions = {
  FEATURE_FLAG_CREATED: 'FEATURE_FLAG_CREATED',
  FEATURE_FLAG_UPDATED: 'FEATURE_FLAG_UPDATED',
  FEATURE_FLAG_OVERRIDE_SET: 'FEATURE_FLAG_OVERRIDE_SET',
  FEATURE_FLAG_PLAN_OVERRIDE_SET: 'FEATURE_FLAG_PLAN_OVERRIDE_SET',
  PRODUCT_ANNOUNCEMENT_CREATED: 'PRODUCT_ANNOUNCEMENT_CREATED',
  PRODUCT_ANNOUNCEMENT_PUBLISHED: 'PRODUCT_ANNOUNCEMENT_PUBLISHED',
  PRODUCT_ANNOUNCEMENT_ARCHIVED: 'PRODUCT_ANNOUNCEMENT_ARCHIVED',
  INCIDENT_COMPOSER_PUBLISHED: 'INCIDENT_COMPOSER_PUBLISHED',
  DOMAIN_ASSISTANT_CHECK_RUN: 'DOMAIN_ASSISTANT_CHECK_RUN',
  BACKUP_RESTORE_PREVIEW_GENERATED: 'BACKUP_RESTORE_PREVIEW_GENERATED',
} as const;

/**
 * Pełny zbiór akcji administracyjnych użyty przez audit viewer (filtr
 * `?category=ADMIN_OPS`).
 */
export const ADMIN_OPS_ACTION_SET: ReadonlySet<string> = new Set([
  ...Object.values(AdminCustomerActions),
  ...Object.values(AdminPlanActions),
  ...Object.values(AdminInvoiceActions),
  ...Object.values(AdminNodeActions),
  ...Object.values(ProvisioningActions),
  ...Object.values(MigrationActions),
  ...Object.values(TicketOpsActions),
  ...Object.values(ProductOpsActions),
  ...Object.values(ControlPlaneMailActions),
]);

export function isAdminOpsAction(action: string): boolean {
  return ADMIN_OPS_ACTION_SET.has(action);
}
