...
const { NATYWNE } = require(join(API, 'rspack.config.js')) as { NATYWNE: string[] };
...
describe('zależności natywne poza paczką bundlera (Rspack)', () => {
  ...
  it('konfiguracja rzeczywiście oddaje je jako commonjs', () => {
    // eslint-disable-next-line @typescript-eslint/no-require-imports
    const cfg = require(join(API, 'rspack.config.js'))({}) as { externals: Array<(a: { request: string }, cb: (e?: unknown, r?: string) => void) => void> };
