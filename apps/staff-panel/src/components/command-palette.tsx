...
/** ADM-4 (parytet staffa) — globalna wyszukiwarka Cmd/Ctrl-K w panelu BOK. */
export function CommandPalette() {
  ...
  const doSearch = useCallback((value: string) => {
    ...
    debounce.current = setTimeout(async () => {
      ...
      setLoading(true);
      const res = await staffGlobalSearchAction(value);
      setResults(res);
      setActive(0);
      setLoading(false);
    }, 220);
  ...
  return (
    <>
      <button
        ...
        aria-label="Szukaj zgłoszenia, klienta, domeny"
        className="flex h-[38px] items-center gap-2.5 rounded-[9px] border border-line-strong bg-card px-3 text-sm text-muted-foreground hover:border-primary md:w-[340px]"
      >
        <Search className="h-[15px] w-[15px] shrink-0" />
        <span className="hidden md:inline">Szukaj zgłoszenia, klienta, domeny…</span>
            ...
            <div className="max-h-[50vh] overflow-y-auto p-2">
              {results.length === 0 ? (
                <p className="px-3 py-8 text-center text-xs text-muted-foreground">
                  {q.trim().length < 2 ? "Wpisz co najmniej 2 znaki." : loading ? "Szukam…" : "Brak wyników."}
                </p>
              ) : (
                results.map((r, i) => {
                  ...
                  return (
                    <button
                      ...
                      className={`flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-left ${
                        i === active ? "bg-cyan-500/15" : "hover:bg-white/5"
                      }`}
                    >
