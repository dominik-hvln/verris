...
/**
 * Wybór załączników w stylu panelu obsługi (port z panelu klienta) zamiast systemowego „Choose files / No file chosen”.
    ...
    <div className={className}>
      ...
      <button
        ...
        className={cx(
          ...
          nadStrefa ? 'border-cyan-400/60 bg-cyan-400/5 text-white' : 'border-white/15 text-muted-foreground hover:border-cyan-400/50 hover:text-white',
        )}
      >
      ...
      {pliki.length ? (
        <ul className="mt-2 space-y-1" aria-label="Wybrane załączniki">
          {pliki.map((f, i) => (
            <li key={`${f.name}-${i}`} className="flex items-center gap-2 rounded-lg border border-white/10 bg-black/35 px-3 py-1.5 text-xs">
