...
interface Props {
  ...
  jobs: JobLite[];
  /** I-22 — w panelu obsługi usługę otwieramy przez CRM klienta. */
...
export function MigrationRowActions({ migrationId, ticketId, needsAttention, jobs, userEmail }: Props) {
  ...
  return (
    <div className="space-y-2">
      <div className="flex flex-wrap items-center justify-end gap-2">
        ...
        {userEmail ? (
          <Link href={`/crm?q=${encodeURIComponent(userEmail)}`} className="text-xs text-indigo-400 hover:underline">
            Klient w CRM
          </Link>
        ) : null}
      </div>
