  ...
  return (
    <div className="flex min-w-[220px] flex-col items-end gap-2">
      <textarea
        ...
        rows={2}
        disabled={pending}
        className="w-full resize-none rounded-md border border-white/10 bg-black/40 px-2 py-1 text-xs text-white placeholder:text-neutral-500"
      />
      ...
      {error ? <p className="max-w-[220px] text-right text-[10px] text-rose-400">{error}</p> : null}
    </div>
  );
}
